...
/* 6. Cuenta el numero de citas que tiene cada doctor*/
...
/* 7. Consulta las citas entre '2022-06-01' y '2022-08-31' */
SELECT * FROM public.cita_medica AS cm
WHERE cm.fecha_programada BETWEEN '2022-06-01' AND '2022-08-31';

/* 8. Cuenta el número de medicamentos en cada formula*/
...
/* 9. Consulta los doctores que sean especialistas en Dermatologia*/
...
/* 10. Consulta las citas entre '12:00' y '14:00' */
SELECT * FROM public.cita_medica AS cm
WHERE cm.hora_programada BETWEEN '12:00' AND '14:00';

/* 11. Consulta las citas que tienen un resgistro de cita*/
SELECT * FROM public.cita_medica as cm, public.registro_cita as rc
WHERE cm.codigo_citamedica = rc.codigo_citamedica;

/* 12. Consulta de los doctores que tengan más de una especializacion */
...
/* 13. Consulta del total de cantidades solicitadas de un medicamento*/
...
/* 14. Consulta de las citas realizadas con el detalle */
...
/* 15. Consulta de las citas realizadas con el detalle y formula medica */
...
/* 16. Consulta de la maxima edad de un paciente*/
SELECT MAX(pa.edad_paciente) as maxima_edad
FROM public.paciente pa;

/* 17. Consulta la minima edad que hay en los paciente */
SELECT MIN(pa.edad_paciente) as minima_edad
FROM public.paciente pa;

/* 18. Promedio de edades en la tabla pacientes */
SELECT AVG(pa.edad_paciente) as promedio_de_edades
FROM public.paciente pa;

/* 19. Seleccion de todos los datos en medicamentos */
SELECT * FROM public.medicamento;

/* 20. Promedio de la cantidad de medicamentos solicitados en las formulas */
SELECT AVG(fm.cantidad_solicitada) AS promedio_de_medicamentos_en_formulas_medicas
FROM public.formula_medica AS fm;
